...
**Last Updated:** 2026-03-04 (synced with `docs/BACKLOG.md`, `docs/changelog/2.3/CHANGELOG.md`, and ast-grep rerun artifacts)
...
> Scope note: Sections 1-6 are the original analyzer snapshot from **2026-02-25**. Current status deltas are reflected in the summary, changelog update section, and priority actions below.
...
## Rerun Baseline (2026-03-03 CST)

Re-ran ast-grep MCP analyzers from `~/code/ast-grep-mcp` against `/Users/alyshialedlie/code/jobs`.

### Rerun Results

| Tool | Rerun Result | Notes |
|------|--------------|-------|
| Complexity (`analyze_complexity`, TypeScript) | **24** functions exceeding thresholds out of **178** across **192** files | Top hotspot remains `frontend/src/hooks/useWebSocketConnection.ts` (cyclomatic 19, cognitive 15, lines 93-142) |
| Code Smells (`detect_code_smells`, TypeScript) | **6,868** findings: **35** large classes, **6,833** magic numbers | High count remains dominated by magic-number findings |
| Security (`detect_security_issues`, TypeScript) | **0** issues | No critical/high/medium/low issues reported |
| Standards (`enforce_standards`, TypeScript, `rule_set=all`) | **109** violations across **36** files | `prefer-const`: 95, `no-console-log`: 14, `no-double-equals`: 0 |
| Orphans (`detect_orphans`) | **458** orphan files, **5,674** orphan functions (764 files analyzed) | Executed with `verify_with_grep=false` for runtime; use caution when comparing to prior snapshot |
| Deduplication (`find_duplication`, Python) | **1** duplication group, **20** potential line savings | 323 constructs analyzed |
| Documentation Sync (`sync_documentation`, TypeScript) | **66.2%** coverage (614/927 documented), 298 undocumented, 37 stale docstrings | Added as supplemental baseline |

### Saved Artifacts

- `docs/output/astgrep/astgrep-complexity-typescript.json`
- `docs/output/astgrep/astgrep-smells-typescript.json`
- `docs/output/astgrep/astgrep-security-typescript.json`
- `docs/output/astgrep/astgrep-standards-typescript.json`
- `docs/output/astgrep/astgrep-orphans.json`
- `docs/output/astgrep/astgrep-duplication-python.json`
- `docs/output/astgrep/astgrep-sync-documentation-typescript.json`

### Delta vs 2026-02-25 Snapshot (Directional)

- Complexity exceedances: **33 -> 24**.
- `no-double-equals` violations: **3 -> 0** in current standards run.
- Smells and orphan totals are not directly comparable due scope/mode differences (notably orphan run without grep verification).

### Documentation Sync Follow-up (2026-03-04 CST)

Ran `sync_documentation` again from `~/code/ast-grep-mcp` and then applied stale-docstring fixes by keeping `@returns` tags and adding explicit function return types (plus malformed JSDoc param cleanup in websocket docs). Follow-up documentation backlog work and additional JSDoc batches were completed in the same session. The latest denominator reflects the current checker pass total (`913` functions).

| Pass | Coverage | Documented | Undocumented | Stale Docstrings |
|------|----------|------------|--------------|------------------|
| Re-check before fixes | 69.5% | 644/927 | 268 | 31 |
| Re-check after fixes | **69.6%** | **645/927** | **267** | **0** |
| Latest rerun after DOC backlog completion | **100.0%** | **913/913** | **0** | **0** |

Fix commit: `4738e17` (`docs: resolve stale docstrings with explicit return types`).
