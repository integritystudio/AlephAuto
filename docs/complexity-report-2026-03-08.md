...
**Project:** jobs
**Date:** 2026-03-08 | **Last Updated:** 2026-03-08 (post-CX14/CX15/CX16 fixes)
...
| Functions exceeding thresholds | 24 → **16** (after fixes) |
...
Overall health is good. No security vulnerabilities detected. All production-code complexity hotspots resolved. Remaining concern is **long test functions** (CX15 — 17 of 22 remaining).
...
| Function | File | Lines | Cyc | Cog | Len | Status |
|----------|------|-------|-----|-----|-----|--------|
| `runTests` | `tests/integration/test-pr-creator.ts` | 414-489 | 10 | 13 | 76 | Open |
| `runTests` | `tests/integration/test-retry-metrics.ts` | 223-291 | 10 | 12 | 69 | Open |
| `runTests` | `tests/integration/test-gitignore-manager.ts` | 438-504 | 9 | 11 | 67 | Open |
| ~~`main`~~ | `tests/integration/test-automated-pipeline.ts` | 20-152 | 9 | 11 | ~~133~~ ~28 | **Fixed** (CX15, v2.3.19) |
| `testDirectoryScanner` | `tests/scripts/test-directory-scanner.ts` | 9-94 | 8 | 9 | 86 | Open |
| `testSkipExisting` | `tests/integration/test-gitignore-manager.ts` | 209-282 | 6 | 9 | 74 | Open |
| ~~`testGitignoreRespect`~~ | `tests/scripts/test-gitignore-respect.ts` | 16-136 | 6 | 9 | ~~121~~ ~26 | **Fixed** (CX15, v2.3.19) |
| `testDryRunMode` | `tests/integration/test-gitignore-manager.ts` | 55-128 | 5 | 9 | 74 | Open |
| `testErrorHandling` | `tests/integration/test-gitignore-manager.ts` | 288-348 | 6 | 7 | 61 | Open |
| ~~`main`~~ | `tests/integration/test-report-generation.ts` | 23-142 | 7 | 5 | ~~120~~ ~30 | **Fixed** (CX15, v2.3.19) |
| `testGitignoreAddition` | `tests/integration/test-gitignore-manager.ts` | 134-203 | 5 | 7 | 70 | Open |
| ~~`testErrorMessages`~~ | `tests/integration/test-error-classification-ui.ts` | 148-214 | 7 | 4 | ~~67~~ 13 | **Fixed** (CX15, v2.3.20) |
| `testSpecificRepositories` | `tests/integration/test-gitignore-manager.ts` | 354-433 | 7 | 3 | 80 | Open |
| `main` (single-job) | `tests/scripts/test-single-job.ts` | 11-84 | 6 | 5 | 74 | Open (DRY fix applied) |
| `main` (repo-cleanup) | `tests/scripts/test-repo-cleanup.ts` | 13-64 | 6 | 2 | 52 | Open |
| `runTests` (pr-creator) | `tests/integration/test-pr-creator.ts` | 110-169 | 4 | 4 | 60 | Open |
| `main` (repomix-fix) | `tests/scripts/test-repomix-fix.ts` | 12-71 | 6 | 2 | 60 | Open |
| `main` (discord-webhook) | `tests/scripts/test-discord-webhook.ts` | 60-152 | 5 | 1 | 93 | Open |
| `main` (sentry-connection) | `tests/scripts/test-sentry-connection.ts` | 16-78 | 4 | 2 | 63 | Open |
| `main` (git-repo-scanner) | `tests/integration/test-git-repo-scanner.ts` | 8-64 | 4 | 2 | 57 | Open |
| `testCachePersistence` | `tests/integration/test-cache-layer.ts` | 136-196 | 5 | 0 | 61 | Open |
| `testCacheInvalidation` | `tests/integration/test-cache-layer.ts` | 201-251 | 3 | 0 | 51 | Open |
...
### Priority 2: Break up long test runners (CX15 — 17 remaining)

17 `runTests`/`main`/test functions in integration tests still exceed 50-line threshold. Top offenders:
- `testDirectoryScanner` (86 lines), `testSpecificRepositories` (80), `runTests` in test-pr-creator (76)
- 6 functions in `test-gitignore-manager.ts` (52-80 lines each)
