...
| ~~FE-M4~~ | ~~P2~~ | ~~**Lookup object allocations in tight loop** — Already resolved: `PIPELINE_ICONS` and `PIPELINE_COLORS` are module-level constants (fixed during `e8c0fab` type centralization refactor).~~ **Done** |
