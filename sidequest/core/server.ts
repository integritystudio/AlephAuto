...
export class SidequestServer extends EventEmitter {
   ...
   */
  protected async _generateCommitMessage(job: Job): Promise<{ title: string; body: string }> {
    return {
   ...
   * Override this method to customize PR details (SC-L5: now protected)
   *
   * @param job Completed job context.
   * @param commitMessage Optional pre-generated commit message.
   * @returns Pull request context payload.
   */
  protected async _generatePRContext(job: Job, commitMessage?: { title: string; body: string }): Promise<{ branchName: string; title: string; body: string; labels: string[] }> {
